...
# mode/yolo/tiering/ci_tests are resolved per-project from data/projects.md for
# ship/scout tasks (fm-project-mode.sh) and recorded into meta as
# mode=/yolo=/tiering=/ci_tests=; secondmate spawns record mode=secondmate,
# yolo=off, tiering=off, ci_tests=off, home=, and projects=.
set -eu
...
launch_template() {
  ...
  case "$harness" in
    ...
    claude) printf '%s' 'CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --model sonnet --dangerously-skip-permissions "$(cat __BRIEF__)"' ;;
    codex)
      if [ "$kind" = secondmate ]; then
...
# Per-project delivery mode + yolo/tiered/ci-tests flags (bin/fm-project-mode.sh;
